                ...
                <div class="bg-black bg-opacity-75 rounded p-4">
                    ...
                    <form action="{{ route('admin.gyms.update', $gym->id) }}" method="POST" enctype="multipart/form-data" id="gymForm">
                        ...
                        <div class="card bg-dark mb-4">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    <label for="images" class="form-label text-light">Upload New Images</label>
                                    <input type="file" class="form-control bg-dark text-light @error('images') is-invalid @enderror" 
                                ...
                                @if($gym->media && count(json_decode($gym->media, true)['images'] ?? []) > 0)
                                    <div class="mb-3">
                                        <label class="form-label text-light">Current Images</label>
                                        <div class="row">
                                            @foreach(json_decode($gym->media, true)['images'] as $index => $image)
                                                <div class="col-md-3 mb-3 image-container" data-index="{{ $index }}">
                                                    <div class="position-relative">
                                                        <img src="{{ asset('storage/' . $image['url']) }}" 
                                                            class="img-thumbnail" 
                                                            alt="{{ $image['caption'] ?? 'Gym image' }}">
                                                        <div class="form-check mt-2">
                                                            <input class="form-check-input" type="checkbox" 
                                                                id="remove_image_{{ $index }}" 
                                                                name="remove_images[]" 
                                                                value="{{ $index }}">
                                                            <label class="form-check-label text-light" for="remove_image_{{ $index }}">
                                                                Remove
                                                            </label>
                                                        </div>
                                                        <div class="mt-2">
                                                        ...
                                                        <div class="form-check mt-2">
                                                            <input class="form-check-input" type="radio" 
                                                                ...
                                                                name="featured_image" 
                                                                value="{{ $index }}"
                                                                {{ ($image['is_featured'] ?? false) ? 'checked' : '' }}>
                                                            <label class="form-check-label text-light" for="featured_image_{{ $index }}">
                                                                Featured
                                                            </label>
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" step="0.01" class="form-control bg-dark text-light" 
                                                name="pricing[day_pass]" 
                                                value="{{ old('pricing.day_pass', $pricing['day_pass'] ?? '15.00') }}">
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label text-light">Currency</label>
                                        <select class="form-select bg-dark text-light" name="pricing[currency]">
                                            <option value="USD" {{ ($pricing['currency'] ?? 'USD') == 'USD' ? 'selected' : '' }}>USD</option>
                                            <option value="EUR" {{ ($pricing['currency'] ?? 'USD') == 'EUR' ? 'selected' : '' }}>EUR</option>
                                            <option value="GBP" {{ ($pricing['currency'] ?? 'USD') == 'GBP' ? 'selected' : '' }}>GBP</option>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" step="0.01" class="form-control bg-dark text-light" 
                                                ...
                                                value="{{ old('pricing.plans.basic.monthly', $pricing['plans']['basic']['monthly'] ?? '39.99') }}">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" step="0.01" class="form-control bg-dark text-light" 
                                                ...
                                                value="{{ old('pricing.plans.basic.annual', $pricing['plans']['basic']['annual'] ?? '399.99') }}">
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" step="0.01" class="form-control bg-dark text-light" 
                                                ...
                                                value="{{ old('pricing.plans.premium.monthly', $pricing['plans']['premium']['monthly'] ?? '59.99') }}">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" step="0.01" class="form-control bg-dark text-light" 
                                                ...
                                                value="{{ old('pricing.plans.premium.annual', $pricing['plans']['premium']['annual'] ?? '599.99') }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('images').addEventListener('change', function(e) {
                ...
                reader.onload = function(e) {
                    ...
                    col.innerHTML = `
                        <div class="position-relative">
                            ...
                            <div class="mt-2">
                                <input type="text" class="form-control bg-dark text-light" 
                                    name="new_image_captions[]" placeholder="Image caption">
                            </div>
                            <div class="form-check mt-2">
                                <input class="form-check-input" type="radio" 
                                    name="featured_image" 
            ...
            const existingPreview = document.getElementById('upload-preview');
            if (existingPreview) {
                existingPreview.remove();
            }
            
            previewContainer.id = 'upload-preview';
            document.getElementById('images').after(previewContainer);
        });
    });
</script>
@endpush